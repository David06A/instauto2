...
export class Utils {
  ...
  static keyBy<T>(collection: T[], key: string): Record<string, T> {
    ...
    collection.reduce(
      (accumulator: Record<string, T>, current: T): Record<string, T> => {
  ...
  static sleep(ms: number, dev = 1): Promise<string> {
    ...
    this._logger.log(
      languageManager.messages.waiting,
      Math.round(msWithDev / 1000),
      'sec'
    );
