...
export class InstagramApi {
  ...
  async startNewPage(overridePage: Page | undefined): Promise<Page> {
    const page = overridePage ?? (await this.browser.newPage());
  ...
  async isActionBlocked(): Promise<boolean> {
    ...
    const actionBlocked =
      (await this.page.$x(pageContent.get(ActionBlocked))).length > 0;
    const tryAgainLater =
      (await this.page.$x(pageContent.get(TryAgainLater))).length > 0;
  ...
  async getUnfollowButton(): Promise<ElementHandle> {
    ...
    const elementHandles3 = await this.page.$x(
      pageContent.get(AriaLabelFollowing)
    );
  ...
  async tryPressAcceptCookies(): Promise<void> {
    if (!this.page) {
      this.logger.warn(`Page not found: ${name}`);
    }
