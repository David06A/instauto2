...
class InstautoCtr {
    constructor(db, browser, page, options) {
        this.db = db;
    ...
    async followUserFollowers(username, { maxFollowsPerUser = 5, skipPrivate = false, enableLikeImages, likeImagesMin, likeImagesMax, }) {
        ...
        for (const follower of followers) {
            try {
                ...
                else if ((this.options.followUserMaxFollowers != null &&
                ...
                else if ((this.options.followUserRatioMax != null &&
    ...
    async initContext() {
        ...
        (0, assert_1.default)(this.options.maxFollowsPerHour * this.botWorkShiftHours >=
            this.options.maxFollowsPerDay, 'Max follows per hour too low compared to max follows per day');
    ...
    async checkReachedFollowedUserDayLimit() {
        const reachedFollowedUserDayLimit = (await this.getNumFollowedUsersThisTimeUnit(this.dayMs)) >=
    ...
    async checkReachedFollowedUserHourLimit() {
        const hasReachedFollowedUserHourLimit = (await this.getNumFollowedUsersThisTimeUnit(this.hourMs)) >=
    ...
    async hasReachedDailyLikesLimit() {
        return ((await this.db.getLikedPhotosLastTimeUnit(this.dayMs)).length >=
    ...
    async haveRecentlyFollowedUser(username) {
        ...
        if (!followedUserEntry)
            return false;
        return (new Date().getTime() - followedUserEntry.time <
    ...
    async likeCurrentUserImagesPageCode({ dryRun: dryRunIn, likeImagesMin, likeImagesMax, }) {
        ...
        for (const image of images) {
            ...
            const section = Array.from(dialog.querySelectorAll('section')).find((s) => s.querySelectorAll('*[aria-label="Like"]')[0] &&
...
const Instauto = async (db, browser, page, options) => {
